...
class post_processing_gui():
    ...
    def __init__(
        ...
        # # Center left
        # center_left_frm = tk.Frame(top_frm, padx=2, pady=2)
        # center_left_frm.pack(side=tk.TOP, anchor="nw", fill="x")

        # small_center_left_frm_1 = tk.Frame(center_left_frm, padx=2, pady=2)
        # small_center_left_frm_2 = tk.Frame(center_left_frm, padx=2, pady=2)

        # small_center_left_frm_1.grid(row=0, column=0, sticky="nsew")
        # small_center_left_frm_2.grid(row=0, column=4, sticky="nsew")

        # IDR_1_label = ttk.Label(center_left_frm, text="First file IDR plot", width=15)
        # IDR_1_label.grid(row=0, column=0, columnspan=1, sticky=tk.W)

        # IDR_2_label = ttk.Label(center_left_frm, text="Second file IDR plot", width=15)
        # IDR_2_label.grid(row=0, column=4, columnspan=1, sticky=tk.W)
        ...
        for c in range(6):
            if c == 1:
                top_left_frm.columnconfigure(c, weight=15)
            else:
...
class pre_processing_gui():
    ...
    def __init__(
        ...
        self.start_steady=self.resting_time*self.sampling_frequency+((self.mvc)/increase_rate)*self.sampling_frequency
        self.end_steady=self.start_steady+steady_time*self.sampling_frequency
    ...
    def Resize(self):
        ...
        start = float(resize_input)*self.sampling_frequency
        end = (self.resting_time*self.sampling_frequency + (self.mvc / self.increase_rate) * self.sampling_frequency + (self.mvc / self.increase_rate) * self.sampling_frequency 
               + self.steady_time * self.sampling_frequency + self.resting_time*self.sampling_frequency - float(resize_input)*self.sampling_frequency)
        ...
        self.start_steady=self.resting_time*self.sampling_frequency-float(resize_input)*self.sampling_frequency+(self.mvc/self.increase_rate)*self.sampling_frequency
        self.end_steady=self.start_steady+self.steady_time*self.sampling_frequency
